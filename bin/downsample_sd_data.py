...
seed = int(sys.argv[6]) if len(sys.argv) > 5 else 112358
...
for lineage_query, lineage_name in zip(lineage_queries, lineage_names):
    ...
    lineage_cases = cases[(cases["updatedate"] <= lineage_metadata["collection_date"].max()) & (cases["updatedate"] >= lineage_metadata["collection_date"].min())]
    cumulative_lineage_cases = lineage_cases.groupby("region").apply(lambda x: x.sort_values("updatedate").iloc[-1])
