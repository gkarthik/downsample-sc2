...
in_ch = guide_trees_ch.join(algn_ch)

process generate_xml {
    input:
        tuple val(out_name), file(tree), file(algn) from ids_algn_ch
        file(xml_template) from params.xml_template
    output:
        file("${out_name}.xml", "${out_name}_final.fa.gz", "${out_name}_final.treefile") into generate_xml_ch
    ...
    script:
        ...
        a=$(get_skygrid_params.py $out_name $tree)
        b=($(echo $a | tr ' ' '\n'))

        beastgen -D "name=${out_name},cutOff=\${b[0]},gridPoints=\${b[1]}" -date_order 2 -date_prefix "|" -date_format "yyyy-MM-dd" -tree ${out_name}_final.treefile $xml_template ${out_name}_final.fa.gz ${out_name}.xml
        """
    stub:
        """
